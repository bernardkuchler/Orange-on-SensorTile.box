...
#if (USE_MOTION_SENSOR_ISM330IS_0 == 1)
...
#if (USE_MOTION_SENSOR_ISM330IS_0 == 1)
static int32_t BSP_ISM330IS_Init(void);
static int32_t BSP_ISM330IS_DeInit(void);
static int32_t BSP_ISM330IS_WriteReg(uint16_t Addr, uint16_t Reg, uint8_t *pdata, uint16_t len);
static int32_t BSP_ISM330IS_ReadReg(uint16_t Addr, uint16_t Reg, uint8_t *pdata, uint16_t len);
#endif
...
static int32_t ISM330IS_0_Probe(uint32_t Functions)
{
  ...
  io_ctx.Address     = 0x0;
  io_ctx.Init        = BSP_ISM330IS_Init;
  io_ctx.DeInit      = BSP_ISM330IS_DeInit;
  io_ctx.ReadReg     = BSP_ISM330IS_ReadReg;
  io_ctx.WriteReg    = BSP_ISM330IS_WriteReg;
  io_ctx.GetTick     = BSP_GetTick;
  ...
  else
  {
    ...
    MotionCtx[ISM330IS_0].Functions = ((uint32_t)cap.Gyro) | ((uint32_t)cap.Acc << 1) | ((uint32_t)cap.Magneto << 2);

    MotionCompObj[ISM330IS_0] = &ism330is_obj_0;
    /* The second cast (void *) is added to bypass Misra R11.3 rule */
    MotionDrv[ISM330IS_0] = (MOTION_SENSOR_CommonDrv_t *)(void *)&ISM330IS_COMMON_Driver;

    if (((Functions & MOTION_ACCELERO) == MOTION_ACCELERO) && (cap.Acc == 1U))
    {
      /* The second cast (void *) is added to bypass Misra R11.3 rule */
      MotionFuncDrv[ISM330IS_0][FunctionIndex[MOTION_ACCELERO]] = (MOTION_SENSOR_FuncDrv_t *)(void *)&ISM330IS_ACC_Driver;

      if (MotionDrv[ISM330IS_0]->Init(MotionCompObj[ISM330IS_0]) != ISM330IS_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else
    ...
    {
      /* The second cast (void *) is added to bypass Misra R11.3 rule */
      MotionFuncDrv[ISM330IS_0][FunctionIndex[MOTION_GYRO]] = (MOTION_SENSOR_FuncDrv_t *)(void *)&ISM330IS_GYRO_Driver;
...
static int32_t BSP_ISM330IS_Init(void)
{
  ...
  /* Configure ISM330IS INT1 pin */
  BSP_DIL24_INT1_GPIO_CLK_ENABLE();
  GPIO_InitStruct.Pin = BSP_DIL24_INT1_PIN;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
  HAL_GPIO_Init(BSP_DIL24_INT1_PORT, &GPIO_InitStruct);

  /* EXTI interrupt init*/
  /* Enable and set EXTI Interrupt priority */
  HAL_NVIC_SetPriority(BSP_DIL24_INT1_EXTI_IRQn, BSP_DIL24_INT1_EXTI_IRQ_PP, BSP_DIL24_INT1_EXTI_IRQ_SP);
  HAL_NVIC_EnableIRQ(BSP_DIL24_INT1_EXTI_IRQn);

  /* Configure ISM330IS INT2 pin */
  BSP_DIL24_INT2_GPIO_CLK_ENABLE();
  GPIO_InitStruct.Pin = BSP_DIL24_INT2_PIN;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
  HAL_GPIO_Init(BSP_DIL24_INT2_PORT, &GPIO_InitStruct);

  /* EXTI interrupt init*/
  /* Enable and set EXTI Interrupt priority */
  HAL_NVIC_SetPriority(BSP_DIL24_INT2_EXTI_IRQn, BSP_DIL24_INT2_EXTI_IRQ_PP, BSP_DIL24_INT2_EXTI_IRQ_SP);
  HAL_NVIC_EnableIRQ(BSP_DIL24_INT2_EXTI_IRQn);
  
  /* Configure ISM330IS CS pin */
  BSP_DIL24_CS_GPIO_CLK_ENABLE();
  HAL_GPIO_WritePin(BSP_DIL24_CS_PORT, BSP_DIL24_CS_PIN, GPIO_PIN_SET);
  
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  
  GPIO_InitStruct.Pin = BSP_DIL24_CS_PIN;
  HAL_GPIO_Init(BSP_DIL24_CS_PORT, &GPIO_InitStruct);
...
static int32_t BSP_ISM330IS_WriteReg(uint16_t Addr, uint16_t Reg, uint8_t *pdata, uint16_t len)
{
  ...
  /* CS Enable */
  HAL_GPIO_WritePin(BSP_DIL24_CS_PORT, BSP_DIL24_CS_PIN, GPIO_PIN_RESET);
