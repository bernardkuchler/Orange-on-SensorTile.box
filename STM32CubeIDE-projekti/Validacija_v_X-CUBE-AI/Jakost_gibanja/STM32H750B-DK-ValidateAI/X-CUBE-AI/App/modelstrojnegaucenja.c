...
#include "modelstrojnegaucenja.h"
#include "modelstrojnegaucenja_data.h"
...
#define AI_NET_OBJ_INSTANCE g_modelstrojnegaucenja
 
#undef AI_MODELSTROJNEGAUCENJA_MODEL_SIGNATURE
#define AI_MODELSTROJNEGAUCENJA_MODEL_SIGNATURE     "d641d6d6a3f0cd3de40c6c49a5ef45c4"
...
#define AI_TOOLS_DATE_TIME   "Wed Jul 12 15:49:18 2023"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_MODELSTROJNEGAUCENJA_N_BATCHES
#define AI_MODELSTROJNEGAUCENJA_N_BATCHES         (1)

static ai_ptr g_modelstrojnegaucenja_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_modelstrojnegaucenja_weights_map[1] = AI_C_ARRAY_INIT;
...
AI_TENSOR_OBJ_DECLARE(
  label_rho, AI_STATIC,
  0, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 1, 3), AI_STRIDE_INIT(4, 4, 4, 12, 12),
...
AI_TENSOR_OBJ_DECLARE(
  ...
  1, 0x0,
  AI_SHAPE_INIT(4, 1, 157, 1, 1), AI_STRIDE_INIT(4, 4, 4, 628, 628),
...
AI_TENSOR_OBJ_DECLARE(
  ...
  2, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 1, 3), AI_STRIDE_INIT(4, 1, 1, 3, 3),
...
AI_TENSOR_OBJ_DECLARE(
  ...
  12, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 1, 266), AI_STRIDE_INIT(4, 4, 4, 1064, 1064),
...
AI_TENSOR_OBJ_DECLARE(
  ...
  13, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 1, 133), AI_STRIDE_INIT(4, 4, 4, 532, 532),
...
AI_LAYER_OBJ_DECLARE(
  ...
  .kernel_type = AI_SVC_KERNEL_SIGMOID, 
  .gamma = 1.0, 
  .coef0 = 1.0, 
  .degree = 3.0, 
  .proba_support = 1, 
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 1644, 1, 1),
    1644, NULL, NULL),
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 648, 1, 1),
    648, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_MODELSTROJNEGAUCENJA_IN_NUM, &float_input_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_MODELSTROJNEGAUCENJA_OUT_NUM, &output_label_output, &output_probability_output),
  &label_layer, 0, NULL)
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool modelstrojnegaucenja_configure_activations(
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool modelstrojnegaucenja_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)
  ...
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_MODELSTROJNEGAUCENJA_MODEL_NAME,
  ...
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
    ...
    err = ai_modelstrojnegaucenja_create(network, AI_MODELSTROJNEGAUCENJA_DATA_CONFIG);
    if (err.type != AI_ERROR_NONE)
        return err;
    if (ai_modelstrojnegaucenja_data_params_get(&params) != true) {
        err = ai_modelstrojnegaucenja_get_error(*network);
        return err;
    }
#if defined(AI_MODELSTROJNEGAUCENJA_DATA_ACTIVATIONS_COUNT)
    if (activations) {
...
#endif
#if defined(AI_MODELSTROJNEGAUCENJA_DATA_WEIGHTS_COUNT)
    if (weights) {
...
#endif
    if (ai_modelstrojnegaucenja_init(*network, &params) != true) {
        err = ai_modelstrojnegaucenja_get_error(*network);
    }
    return err;
}
...
AI_API_ENTRY
ai_buffer* ai_modelstrojnegaucenja_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
...
AI_API_ENTRY
ai_buffer* ai_modelstrojnegaucenja_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    ((ai_network *)network)->magic = AI_MAGIC_CONTEXT_TOKEN;
